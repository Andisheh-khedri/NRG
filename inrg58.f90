...
!----------------------------------------------------------------------
integer, parameter:: mmax=40,izmax=20,itmaxdim=512,nchannel=1,echarge=7,iw1maxdim=4*512,Nbtotal=40,ie0max=100&
                     ,tnumstates=1000,numgap=0
...
integer*8::m,n,i,j,s,k,ss,kk,openstatus,nmax,d0,dbc,dmax,dof,icut,icdint&
          ...
          ,it,itcounter,start,itmaximum,drnmax,discounter,nii,nup,ndown,degeneracy,iw1max,iomega0
double precision::l,e,Vl,Vr,u,width,gama,ans1,ans2,ans3,ansn,emin,asumFcorr,sumFcorr,justsumFcorr&
                  ...
                  ,barbeta,Etot,EEtot,Ecut,error,Gre,alphatem,bfactor,dfactor,alphaw1,newarea1,newarea2
...
double precision::zavscimp,collector,ndcollector,ndzav,zavspectral,nddcollector,nddzav,nbocollector&
                  ,collector2,collector3,collector4,collector5,collector6,collector7,collector8,collector9,collector10,eta
...
allocate(ddmat(0:mmax,1:2*(mmax+1)+1,1:400,1:400),bpbmat(0:mmax,1:2*(mmax+1)+1,1:400,1:400))
!-----------------------------------------------------------------------
...
if(rank==0)then
!------------------------------------------------------------------------------------------------
open(unit=2,file="kspectrallsqrt2w01ge0epm6T0.0001g.txt",status="new",action="write",position="rewind",IOSTAT=openstatus)
!------------------------------------------------------------------------------------------------
endif
!------------------------------------------------------------------------------------------------
Do iomega0=1,1
...
w0=1.0d+00*gama!*10000000.0d+00!0.00025d+00!1.0d+00*gama
lambda=(sqrt(2.0d+00))*w0
!-------------------------------------------------------------------------------------
!-------------------------------------------------------------------------------------
...
!e=200.0d+00*0.1d+00*gama*(((eint-51.0)*exp((abs(51.0- eint)-50.0)/230.0d+00)/50.0d+00))   !Impurity energy
!e=-(w0*(0.30d+00+(iomega0-1)*0.1d+00))+((lambda**2)/w0)!-2.0d+00*w0
e=(6.0d+00*w0)+((lambda**2)/w0)
e0(eint)=e                           
!-----------------------------------------------------------------------
...
Do k=1,Nbtotal+1
if(k==s)then
hm(s,k)=(n-0.0d+00*0.50d+00)*e+(s-1)*w0
elseif(k==s-1)then
hm(s,k)=(n-0.0d+00*0.50d+00)*lambda*sqrt(1.0d+00*(k))
elseif(k==s+1)then
hm(s,k)=(n-0.0d+00*0.50d+00)*lambda*sqrt(1.0d+00*(s))
else
hm(s,k)=0.0d+00
endif
end do !k
...
!-------------------------------------------------------------------------------------------
 	...
 	drt=drt+dr(n)
        ...
        print*,iz,m,dtotal,drt,dmax,drnmax,Ecut,emin-Htotal(1)
!-----------------------------------------------------------------------
!-----------------------------------------------------------------------
endif
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
itcounter=0
Do it=1,1!itmaxdim!itmaxdim-15,itmaxdim-15!
...
Tem(it)=0.0001d+00*w0!l**(-(mmax-3.0d+00)/2.0d+00)!0.4d+00*gama!
...
! Best Shell:
!-----------------------------------------------------------------------
Do it=1,1!itmaxdim-15,itmaxdim-15!1,itmaximum!itmaxdim-15,itmaxdim-15!
...
!-----------------------------------------------------------------------
Do iw1=1,iw1maxdim/2
!-----------------------------------------------------------------------
w1(iw1)=1.0d+00*(l**(-18.d+00*(iw1-1)/(((iw1maxdim/2)-1)*1.0d+00)))
!1.0d+00*(l**(-24.d+00*(iw1-1)/(((iw1maxdim/2)-1)*1.0d+00)))
...
!----------checking!
!-------------------------------------------------------------------------------------------------------------------------
if(w1shell(iw1)==14)then
!write(2,*)iw1,w1(iw1)/gama,w1shell(iw1),n,s,k,Hstore(w1shell(iw1),n+1,s)-Hstore(w1shell(iw1),n,k),deltaw1(iw1),adeltaw1(iw1)!&
!          ,w1(iw1)-(Hstore(w1shell(iw1),n+1,s)-Hstore(w1shell(iw1),n,k))
!         ,(exp(-(((w1(iw1)-(Hstore(w1shell(iw1),n+1,s)-Hstore(w1shell(iw1),n,k)))/(dfactor))**2)))
endif
!-------------------------------------------------------------------------------------------------------------------------
end do !k
end do !s
...
newspectral(it,iw1)=-aimag(1.0d+00/(w1(iw1)-1.0d+00*e0(eint)+ic*gama-phselfenergy(it,iw1)))
...
anewspectral(it,iw1)=-aimag(1.0d+00/(w1(iw1)-1.0d+00*e0(eint)+ic*gama-aphselfenergy(it,iw1)))
!-----------------------------------------------------------------------
...
! checking the normalization of spectral function:
!-----------------------------------------------------------------------
if(iw1>2 .and. iw1<2*iw1max-4)then
newarea1=newarea1-(w1(iw1)-w1(iw1-1))*(newSpectral(it,iw1)+newSpectral(it,iw1-1))*0.50d+00
newarea2=newarea2-(w1(iw1)-w1(iw1-1))*(anewSpectral(it,iw1)+anewSpectral(it,iw1-1))*0.50d+00
endif
!-----------------------------------------------------------------------
!-----------------------------------------------------------------------
!write(2,*)it,Tem(it)/gama,iw1,w1(iw1)/gama,w1shell(iw1),pi*gama*zSpectral(it,iw1)/(1.0d+00*izmax)&
!         ,pi*gama*zaSpectral(it,iw1)/(1.0d+00*izmax),real(DotgR(it,iw1)),real(aDotgR(it,iw1))&
!         ,aimag(DotgR(it,iw1)),aimag(aDotgR(it,iw1))
!-----------------------------------------------------------------------
write(2,*)it,Tem(it)/gama,iw1,w1(iw1)/gama,w1shell(iw1),pi*gama*zSpectral(it,iw1)&
         ...
         ,w1(iw1)/dble(tm(w1shell(iw1)-1)),area1,area2,iomega0,w0/gama,lambda/w0&
         ,(e0(eint)-(((lambda)**2)/w0))/w0,iomega0,newarea1/pi,newarea2/pi
!-----------------------------------------------------------------------
if(iw1==iw1max-1)then
!write(2,*)it,Tem(it)/gama,iw1,w1(iw1)/gama,w1shell(iw1),pi*gama*zSpectral(it,iw1)&
