...
! *********************************************************************
!*In comparison to last version (22), the calculation of 
! specific heat has been included 
! calculating C of c-band by inserting u=0 and g=0 (not successful!!!!)
!checking the cbands
! *********************************************************************
! Implementation of z-averaging 
! *********************************************************************
! Prrallel version :D (for arbitrary T grid)
! izmax=number of processors we are going to use
! *********************************************************************
! compard to inrg39.f90 mpi has been used instead of mpi-send-recieve
! *********************************************************************
! compard to inrg42.f90 saving the informations for each chain-lenght 
...
!----------------------------------------------------------------------
integer, parameter:: mmax=40,izmax=4,itmaxdim=512,nchannel=1,echarge=7,iw1maxdim=512,Nbtotal=0,ie0max=100
double precision,PARAMETER ::pi = 3.1415926535897931d+0
integer*8::m,n,i,j,s,k,ss,kk,openstatus,nmax,d0,dbc,dmax,dof&
          ,nmin,imin,drt,eint,mincheck,grss,maxsort&
          ,dtotal,n1,i1,nexcitedprint,maxsorting,deltam,maxdeltam&
          ,cdint,nexprint,mmin,rnmax,izcount,iw1,nb&
          ,it,itcounter,start,itmaximum,drnmax,discounter,nii,nup,ndown,degeneracy,iw1max
double precision::l,e,Vl,Vr,u,width,gama,ans1,ans2,ans3,ansn,emin&
                  ,ex1,tmr,tml,alpha,zparcheck,area1,area2,lambda,w0,frganagama,mapgef&
                  ,barbeta,Etot,EEtot,Ecut,error,Gre,alphatem,bfactor,dfactor,alphaw1
double precision,allocatable,dimension(:,:,:)::h,Trb,nd,ndb,ddagger,dbdagger,nboson,nbeboson
...
double precision,dimension(2,itmaxdim)::Hav,HHav,BEntropy,zpar
double precision,dimension(2,itmaxdim)::Netot
double precision::zavscimp,collector,ndcollector,ndzav,zavspectral,nddcollector,nddzav,nbocollector
double precision,dimension(2,itmaxdim)::scimp
double precision,dimension(itmaxdim)::ndtot,nTboson
!===============================================================
! Tem grid:
...
double precision,dimension(0:mmax,0:((nchannel*(mmax+1))+1),1:5000)::Hstore,ndmat,nbomat
double precision,allocatable,dimension(:,:,:,:)::Trstore,ddmat
...
double precision,dimension(1:itmaxdim,1:iw1maxdim)::spectral,sumrule,aspectral
...
double precision,dimension(1:ie0max,1:itmaxdim)::nde0,ndde0
...
!allocate(ddmat(0:mmax,1:2*(mmax+1)+1,1:1000,1:1000))
!-----------------------------------------------------------------------
...
if(rank==0)then
!------------------------------------------------------------------------------------------------
open(unit=2,file="k4znb8.txt",status="new",action="write",position="rewind",IOSTAT=openstatus)
!------------------------------------------------------------------------------------------------
endif
!------------------------------------------------------------------------------------------------
!------------------------------------------------------------------------------------------------
...
w0=0.0d+00*width!100.0d+00*gama
lambda=0.0d+00*w0
!-----------------------------------------------------------------------
...
frganagama=gama*(exp(((-((lambda/w0)**2))*(((4.0d+00*gama/(pi*w0))*log(gama/w0))&
                        ...
                        /((1.0d+00+(gama/w0)**2)**2)))
!-------------------------------------------------------------------------------------
! approximated effective Tunneling rate from IRLM-mapping (lambda>>1): 
!-------------------------------------------------------------------------------------
...
print*,"approximated effective Tunneling rate from IRLM-maping (lambda>>1)=",mapgef/gama
!-------------------------------------------------------------------------------------------------
!e=0.0d+00!lambda**2/w0!0.0d+00*gama  !40*gama*((( eint-51.0)*exp((abs(51.0- eint)-50.0)/23)/50.0)) !Impurity energy   
e=0.10d+00*gama*((( eint-21.0)*exp((abs(21.0- eint)-20.0)/23.0d+00)/20.0d+00)) !Impurity energy
e0(eint)=e                           
!-----------------------------------------------------------------------
...
elseif(cdint==1)then
  ...
  elseif(nchannel==1)then
    ...
    if(i==j)then
    if(i<Nbtotal+2)then
    h(1,i,i)=(i-1)*w0!H0local(1,i-1)
    else
    h(1,i,i)=(i-Nbtotal-2)*w0!H0local(0,i-Nbtotal-2)
    endif
    elseif(i==j-Nbtotal-1 .or. i==j+Nbtotal+1)then
    h(1,i,j)=tml
    endif
...
!-------------------------------------------------------------------------------------------
        !if(dmax<1000 .and.discounter==0)then        
        !Ecut=100.0d+00*tml 
        !else
        Ecut=50.0d+00*tml!200.0d+00*tml   !
 	...
 	drt=drt+dr(n)
        ...
        !print*,m,dtotal,drt,dmax,drnmax
	!-----------------------------------------------------------------------
	! printing the first few excited energies:
	!-----------------------------------------------------------------------
	if(2*int(m/2.0)==m)then
	Do ss=1,min(10,dr(nmin))
!  	write(1,*),m,nmin,(hb(m,ss)-emin)/(l**(-(m-1)/2.0)),(hb(m+1,ss)-emin)/(l**(-(m-1)/2.0))&
!  	          ,(hb(m+2,ss)-emin)/(l**(-(m-1)/2.0))&
!  	          ,(hb(m+3,ss)-emin)/(l**(-(m-1)/2.0))&
...
if(ss>0 .and. d(n)>0 .and. n-nii>=0 )then
...
!-----------------------------------------------------------------------
!-----------------------------------------------------------------------
allocate(Amatrix(1:ss,1:d(n)),Bmatrix(1:ss,1:ss),Dmatrix(1:d(n),1:ss))

Amatrix(1:ss,1:d(n))=Trb(n,(ndown+1):(nup),1:d(n))
Bmatrix(1:ss,1:ss)=ndb(n-nii,:,:)

CALL dgemm ('T','N',d(n),ss,ss,alphaconst,Amatrix,ss,Bmatrix,ss,betaconst,Dmatrix,d(n)) 

deallocate(Amatrix,Bmatrix)

allocate(Amatrix(1:d(n),1:ss),Bmatrix(1:ss,1:d(n)))

Amatrix=Dmatrix
Bmatrix=Trb(n,(ndown+1):nup,1:d(n))

deallocate(Dmatrix)

allocate(Dmatrix(1:d(n),1:d(n)))

CALL dgemm ('N','N',d(n),d(n),ss,alphaconst,Amatrix,d(n),Bmatrix,ss,betaconst,Dmatrix,d(n)) 


nd(n,1:d(n),1:d(n))=nd(n,1:d(n),1:d(n))+Dmatrix
...
nboson(n,1:d(n),1:d(n))=nboson(n,1:d(n),1:d(n))+Dmatrix
...
if(cdint>1)then
do n=0,nmax-1
if(d(n+1)>0 .and. d(n)>0)then
Do i=1,4
!-----------------------------------------------------------------------
if(ssmat(i,n)>0 .and. ssmat(i,n+1)>0 .and. n-niimat(i)>-1)then

allocate(Amatrix(1:ssmat(i,n+1),1:d(n+1)),Bmatrix(1:ssmat(i,n+1),1:ssmat(i,n)),Dmatrix(1:d(n+1),1:ssmat(i,n)))

Amatrix(1:ssmat(i,n+1),1:d(n+1))=Trb(n+1,(ndownmat(i,n+1)+1):(nupmat(i,n+1)),1:d(n+1))
Bmatrix(1:ssmat(i,n+1),1:ssmat(i,n))=dbdagger(n-niimat(i),1:ssmat(i,n+1),1:ssmat(i,n))

CALL dgemm ('T','N',d(n+1),ssmat(i,n),ssmat(i,n+1),alphaconst,Amatrix,ssmat(i,n+1),Bmatrix,ssmat(i,n+1),betaconst,Dmatrix,d(n+1))


deallocate(Amatrix,Bmatrix)
allocate(Amatrix(1:d(n+1),1:ssmat(i,n)),Bmatrix(1:ssmat(i,n),1:d(n)))

Amatrix=Dmatrix
Bmatrix=Trb(n,(ndownmat(i,n)+1):nupmat(i,n),1:d(n))
deallocate(Dmatrix)
allocate(Dmatrix(1:d(n+1),1:d(n)))

CALL dgemm ('N','N',d(n+1),d(n),ssmat(i,n),alphaconst,Amatrix,d(n+1),Bmatrix,ssmat(i,n),betaconst,Dmatrix,d(n+1)) 

ddagger(n,1:d(n+1),1:d(n))=ddagger(n,1:d(n+1),1:d(n))+Dmatrix(1:d(n+1),1:d(n))
!-----------------------------------------------------------------------
deallocate(Amatrix,Bmatrix,Dmatrix)
endif
!-----------------------------------------------------------------------
...
if(cdint>1)then
!-----------------------------------------------------------------------
deallocate(ndb,dbdagger,nbeboson)
...
if(n<nmax)then
dbdagger(n,1:d(n+1),1:d(n))=ddagger(n,1:d(n+1),1:d(n))
!ddmat(m,n,1:dr(n+1),1:dr(n))=ddagger(n,1:dr(n+1),1:dr(n))
endif
end do
!-----------------------------------------------------------------------
deallocate(nd,ddagger,nboson)
!-----------------------------------------------------------------------
endif !cdint>1
!-----------------------------------------------------------------------
!-----------------------------------------------------------------------
...
Tem(it)=10.0d+00*(l**(-19.d+00*(it-1)/((itmaxdim-1)*1.0d+00)))!l**(-(mmax-3)/2.0d+00)   !10.0d+00*(l**(-19.d+00*(it-1)/((itmaxdim-1)*1.0d+00)))
if(Tem(it)<dble(tm(mmax)))then
...
!-----------------------------------------------------------------------
!-----------------------------------------------------------------------
beta(it)=1.0d+0/Tem(it)
!-----------------------------------------------------------------------
! Initialization
!-----------------------------------------------------------------------
zpar(cdint,it)=0.0d+0

Hav(cdint,it)=0.0d+0
HHav(cdint,it)=0.0d+0
Netot(cdint,it)=0.0d+0
ndtot(it)=0.0d+00
nTboson(it)=0.0d+00
!-----------------------------------------------------------------------
! Netot(1,0)=nchannel/2.0d+0
! Hav(1,0)=0.0d+0
! HHav(1,0)=0.0d+0
!-----------------------------------------------------------------------
Do n=0,(nchannel*(Tshell(it)+1)+cdint-1)
Do s=1,dstore(cdint,Tshell(it),n)
...
!-----------------------------------------------------------------------
!-----------------------------------------------------------------------
if(cdint==1 .and. Tshell(it)==0)then
  if(nchannel==1)then
  zpar(cdint,it)=4.0d+0
  elseif(nchannel==2)then
  zpar(cdint,it)=8.0d+0
  endif
endif
!-----------------------------------------------------------------------
if(cdint>1)then
...
!-----------------------------------------------------------------------
!if(cdint>1.and.m>0)then
!write(1,*),m,it,Tem(it),scimp(2,it)-scimp(1,it)
!endif
!-----------------------------------------------------------------------        
!-----------------------------------------------------------------------
if(cdint>1)then
...
Do n=0,nchannel*(m+1)
Do s=1,dstore(2,m,n+1)
Do k=1,dstore(2,m,n)
!spectral(1,1)=spectral(1,1)+((ddmat(m,n,s,k))**2)*(exp(-beta(it)*(Hstore(m,n+1,s)-eground(2,m)))&
!                                                  +exp(-beta(it)*(Hstore(m,n,k)-eground(2,m))))
...
if(abs(Hstore(m,n,s)-eground(2,m))<10.0d+00**(-20.0d+00))then
...
!-----------------------------------------------------------------------
Do iw1=1,1!iw1maxdim
!-----------------------------------------------------------------------
Spectral(it,iw1)=0.0d+00
sumrule(it,iw1)=0.0d+00
aSpectral(it,iw1)=0.0d+00
!-----------------------------------------------------------------------
w1(iw1)=10.0d+00*(l**(-19.d+00*(iw1-1)/((iw1maxdim-1)*1.0d+00)))
if(w1(iw1)<dble(tm(mmax)))then
...
!-----------------------------------------------------------------------
dfactor=1.9d+00*(dble(tm(w1shell(iw1)-1)))
!-----------------------------------------------------------------------
Do n=0,nchannel*(w1shell(iw1)+1)
Do s=1,dstore(2,w1shell(iw1),n+1)
do k=1,dstore(2,w1shell(iw1),n)
...
!Spectral(it,iw1)=Spectral(it,iw1)+((ddmat(w1shell(iw1),n,s,k))**2)&
!                                 *((exp(-beta(it)*(Hstore(w1shell(iw1),n+1,s)-eground(2,w1shell(iw1)))))&
...
adeltaw1(iw1)=((exp(-(bfactor**2)/4.0d+00))/(bfactor*(sqrt(pi))*abs(Hstore(w1shell(iw1),n+1,s)-Hstore(w1shell(iw1),n,k))))&
               *(exp(-((log(abs(w1(iw1)/(Hstore(w1shell(iw1),n+1,s)-Hstore(w1shell(iw1),n,k)))))**2)))

!aSpectral(it,iw1)=aSpectral(it,iw1)+((ddmat(w1shell(iw1),n,s,k))**2)&
!                                 *((exp(-beta(it)*(Hstore(w1shell(iw1),n+1,s)-eground(2,w1shell(iw1)))))&
...
!-----------------------------------------------------------------------
if(rank.eq.0) then
!write(1,*)it,Tem(it)/gama,iw1,w1(iw1)/gama,w1shell(iw1),Spectral(it,iw1),aSpectral(it,iw1),sumrule(it,iw1),area1,area2
!write(2,*)iw1,w1(iw1)/gama,pi*gama*Spectral(it,iw1),aSpectral(it,iw1),sumrule(it,iw1),area1,area2,Tem(it)/gama
end if
!-----------------------------------------------------------------------
end do !iw1
!-----------------------------------------------------------------------
...
zavspectral=Spectral(it,iw1)
!call MPI_Reduce(zavspectral,collector,1,MPI_double_precision,MPI_SUM,0, MPI_COMM_WORLD, ierr)
!-----------------------------------------------------------------------
!-----------------------------------------------------------------------
if(rank.eq.0) then
!write(2,*)it,Tem(it)/gama,iw1,w1(iw1)/gama,w1shell(iw1),pi*gama*collector/(1.0d+00*izmax)
end if
!-----------------------------------------------------------------------
!-----------------------------------------------------------------------
end do !iw1
...
!-----------------------------------------------------------------------
!nde0(21,it)=0.50d+00
!ndde0(21,it)=0.50d+00
!-----------------------------------------------------------------------
efgamma(it)=-1.0d+00*(e0(22)-e0(21))/(pi*(nde0(22,it)-nde0(21,it)))  
efgammadd(it)=-1.0d+00*(e0(22)-e0(21))/(pi*(ndde0(22,it)-ndde0(21,it)))  
!-----------------------------------------------------------------------
call MPI_Reduce(efgamma(it),ndcollector,1,MPI_double_precision,MPI_SUM,0, MPI_COMM_WORLD, ierr)
!-----------------------------------------------------------------------
call MPI_Reduce(efgammadd(it),nddcollector,1,MPI_double_precision,MPI_SUM,0, MPI_COMM_WORLD, ierr)
!-----------------------------------------------------------------------
!-----------------------------------------------------------------------
!-----------------------------------------------------------------------
! Effective Tunneling Rate:
!-----------------------------------------------------------------------
!efgamma(it)=ndcollector/(1.0d+00*izmax)
!efgammadd(it)=nddcollector/(1.0d+00*izmax)
if(rank.eq.0) then
write(2,*)e0(22)/gama,it,Tem(it)/gama,ndcollector/(1.0d+00*izmax*gama),nddcollector/(1.0d+00*izmax*gama),frganagama/gama,mapgef/gama
endif
!-----------------------------------------------------------------------
!-----------------------------------------------------------------------
end do
